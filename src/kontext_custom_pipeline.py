...
class FluxKontextPipelineWithPhotoEncoderAddTokens(
    ...
    def prepare_latents(
        ...
        if image_B.shape != image_A.shape:
            raise ValueError(f"Image A and Image B must have the same shape, but got {image_A.shape} and {image_B.shape}.")
    ...
    def __call__(
        ...
        if (
            ...
            and not (isinstance(image_A, torch.Tensor) and image_A.size(1) == self.latent_channels)
            and not (isinstance(image_B, torch.Tensor) and image_B.size(1) == self.latent_channels)
        ):
        ...
        if self.detail_encoder is not None:
            ...
            for index, token_id in enumerate(input_ids.tolist()):
                if token_id == image_B_token_id:
                    ...
                    break
            class_tokens_A_mask = [
                True if class_token_A_index <= i < class_token_A_index + 1 else False for i in range(prompt_embeds.shape[1])
            ]
            class_tokens_B_mask = [
                True if class_token_B_index <= i < class_token_B_index + 1 else False for i in range(prompt_embeds.shape[1])
            ]
